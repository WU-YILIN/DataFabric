...
const SandboxExperimentation = () => {
  ...
  const onCreate = async (event: FormEvent) => {
    ...
    if (!parsedConfig.ok) {
      setError(`ʵʧ: ${parsedConfig.error}`)
      return
    }
    if (!createForm.source_id) {
      setError('ʵʧ: ѡ source')
      return
    }
    ...
    try {
      ...
      setError(e?.response?.data?.message ?? 'ʵʧ')
    } finally {
      setLoading(false)
    }
  }
  ...
  const onRun = async () => {
    ...
    if (!parsedCandidates.ok || !Array.isArray(parsedCandidates.data)) {
      setError('ʵʧ: candidate_json  JSON')
      return
    }
  ...
  const onPromote = async () => {
    ...
    try {
      ...
      setError(e?.response?.data?.message ?? 'ƹʧ')
    } finally {
      setLoading(false)
    }
  }
